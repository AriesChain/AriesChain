// Package qctstats implements the network stats reporting service.
package qctstats
...
import (
	...
	"github.com/quickchainproject/quickchain/common/mclock"
...
type Service struct {
	...
	eth    *qct.Ethereum      // Full Ethereum service if monitoring a full node
...
func New(url string, ethServ *qct.Ethereum, lesServ *les.LightEthereum) (*Service, error) {
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["eth"]; info != nil {
		network = fmt.Sprintf("%d", info.(*qct.NodeInfo).Network)
		protocol = fmt.Sprintf("eth/%d", qct.ProtocolVersions[0])
	} else {
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the qctstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to qctstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to qctstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to qctstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	log.Trace("Sending pending transactions to qctstats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	log.Trace("Sending node details to qctstats")
