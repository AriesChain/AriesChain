...
// NewProtocolManager returns a new quickchain sub protocol manager. The Ethereum sub protocol manages peers capable
// with the quickchain network.
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Light Quickchain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Quickchain peer connected", "name", p.Name())
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case GetHeaderProofsMsg:
		...
		trieDb := trie.NewDatabase(qctdb.NewTable(pm.chainDb, light.ChtTablePrefix))
		for _, req := range req.Reqs {
		...
		for _, req := range req.Reqs {
			if auxTrie == nil || req.Type != lastType || req.TrieIdx != lastIdx {
				...
				if root, prefix = pm.getHelperTrie(req.Type, req.TrieIdx); root != (common.Hash{}) {
					auxTrie, _ = trie.New(root, trie.NewDatabase(qctdb.NewTable(pm.chainDb, prefix)))
				}
			}
