...
import (
	...
	"github.com/quickchainproject/quickchain/internal/jsre"
	"github.com/quickchainproject/quickchain/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jqct", struct{}{})

	jethObj, _ := c.jsre.Get("jqct")
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jqct);"); err != nil {
	...
	flatten := "var let = web3.let; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	if c.prompter != nil {
		...
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jqct.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jqct.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jqct.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jqct.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines let.getBalance(let.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	// Print some generic Quickchain metadata
	fmt.Fprintf(c.printer, "Welcome to the Quickchain JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + let.coinbase);
		console.log("at block: " + let.blockNumber + " (" + new Date(1000 * let.getBlock(let.blockNumber).timestamp) + ")");
