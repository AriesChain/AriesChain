...
func TestNodeIterator(t *testing.T) {
	var (
		fulldb, _  = qctdb.NewMemDatabase()
		lightdb, _ = qctdb.NewMemDatabase()
		gspec      = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis    = gspec.MustCommit(fulldb)
	)
	gspec.MustCommit(lightdb)
	blockchain, _ := core.NewBlockChain(fulldb, nil, params.TestChainConfig, qcthash.NewFullFaker(), vm.Config{})
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, qcthash.NewFaker(), fulldb, 4, testChainGen)
