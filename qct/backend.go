...
import (
	...
	bftBackend "github.com/quickchainproject/quickchain/consensus/bft/backend"
	dbft "github.com/quickchainproject/quickchain/consensus/dbft"
	dbftBackend "github.com/quickchainproject/quickchain/consensus/dbft/backend"
	...
	"github.com/quickchainproject/quickchain/internal/qctapi"
	...
	"github.com/quickchainproject/quickchain/miner"
...
type Ethereum struct {
	...
	// DB interfaces
	chainDb qctdb.Database // Block chain database
	...
	networkId     uint64
	netRPCService *qctapi.PublicNetAPI
...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	// force to set the bft etherbase to node key address
	...
	log.Info("Initialising Quickchain protocol", "versions", ProtocolVersions, "network", config.NetworkId)

	if !config.SkipBcVersionCheck {
		bcVersion := core.GetBlockChainVersion(chainDb)
		if bcVersion != core.BlockChainVersion && bcVersion != 0 {
			return nil, fmt.Errorf("Blockchain DB version mismatch (%d / %d). Run quickchain upgradedb.\n", bcVersion, core.BlockChainVersion)
		}
...
func CreateDB(ctx *node.ServiceContext, config *Config, name string) (qctdb.Database, error) {
	db, err := ctx.OpenDatabase(name, config.DatabaseCache, config.DatabaseHandles)
	if err != nil {
		return nil, err
	}
	if db, ok := db.(*qctdb.LDBDatabase); ok {
...
func CreateConsensusEngine(ctx *node.ServiceContext, config *Config, chainConfig *params.ChainConfig, db qctdb.Database) consensus.Engine {
	...
	if chainConfig.BFT != nil {
		if chainConfig.BFT.Epoch != 0 {
			config.BFT.Epoch = chainConfig.BFT.Epoch
		}
...
// APIs returns the collection of RPC services the quickchain package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *Ethereum) APIs() []rpc.API {
	apis := qctapi.GetAPIs(s.ApiBackend)
...
func (s *Ethereum) ChainDb() qctdb.Database            { return s.chainDb }
...
func (s *Ethereum) Start(srvr *p2p.Server) error {
	...
	s.netRPCService = qctapi.NewPublicNetAPI(srvr, s.NetVersion())
