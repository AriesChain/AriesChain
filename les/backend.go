...
import (
	...
	rpc "github.com/quickchainproject/quickchain/rpc"
)
...
func New(ctx *node.ServiceContext, config *qct.Config) (*LightEthereum, error) {
	chainDb, err := qct.CreateDB(ctx, config, "lightchaindata")
	if err != nil {
		return nil, err
	}
	...
	leth := &LightEthereum{
		...
		engine:           qct.CreateConsensusEngine(ctx, config, chainConfig, chainDb),
		shutdownChan:     make(chan bool),
		networkId:        config.NetworkId,
		bloomRequests:    make(chan chan *bloombits.Retrieval),
		bloomIndexer:     qct.NewBloomIndexer(chainDb, light.BloomTrieFrequency),
...
func (s *LightEthereum) APIs() []rpc.API {
	return append(qctapi.GetAPIs(s.ApiBackend), []rpc.API{
		{
			Namespace: "eth",
			Version:   "1.0",
...
func (s *LightEthereum) Start(srvr *p2p.Server) error {
	...
	s.netRPCService = qctapi.NewPublicNetAPI(srvr, s.networkId)
