...
func (s *PublicAccountAPI) Accounts() []string {
	...
	for _, wallet := range s.am.Wallets() {
		for _, account := range wallet.Accounts() {
			addresses = append(addresses, Addr2QctStr(account.Address))
		}
	}
	return addresses
}
...
func (s *PrivateAccountAPI) NewAccount(password string) (string, error) {
	...
	if err == nil {
		return Addr2QctStr(acc.Address), nil
	}
	return Addr2QctStr(common.Address{}), err
}
...
//convert 0x... to QCT...
func Addr2QctStr(addr common.Address) string {
	hex := addr.Hex()
	return "QCT" + hex[2:len(hex)]
}

func Addr2QctStrPtr(addr *common.Address) *string {
	if addr == nil {
		return &common.ZeroAddressString
	}

	hex := (*addr).Hex()
	hex = "QCT" + hex[2:len(hex)]
	return &hex
}
...
func (s *PrivateAccountAPI) signTransaction(ctx context.Context, args SendTxArgs, passwd string) (*types.Transaction, error) {
	...
	account := accounts.Account{Address: str2Addr(args.From)}
...
func (s *PrivateAccountAPI) SendTransaction(ctx context.Context, args SendTxArgs, passwd string) (common.Hash, error) {
	if args.Nonce == nil {
		...
		s.nonceLock.LockAddr(str2Addr(args.From))
		defer s.nonceLock.UnlockAddr(str2Addr(args.From))
	}
...
//
// https://github.com/quickchainproject/quickchain/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with qct_sign and personal_sign. As such it recovers
// the address of:
...
//
// https://github.com/quickchainproject/quickchain/wiki/Management-APIs#personal_ecRecover
...
func (s *PublicBlockChainAPI) GetBalance(ctx context.Context, address string, blockNr rpc.BlockNumber) (*big.Int, error) {
	...
	b := state.GetBalance(str2Addr(address))
...
func (s *PublicBlockChainAPI) rpcOutputBlock(b *types.Block, inclTx bool, fullTx bool) (map[string]interface{}, error) {
	...
	fields := map[string]interface{}{
		...
		"stateRoot":        head.Root,
		"signer":           Addr2QctStr(head.Coinbase),
...
func newRPCTransaction(tx *types.Transaction, blockHash common.Hash, blockNumber uint64, index uint64) *RPCTransaction {
	...
	result := &RPCTransaction{
		Type:     tx.Type(),
		From:     Addr2QctStr(from),
		Gas:      hexutil.Uint64(tx.Gas()),
		GasPrice: (*hexutil.Big)(tx.GasPrice()),
		Hash:     tx.Hash(),
		Input:    hexutil.Bytes(tx.Data()),
		Nonce:    hexutil.Uint64(tx.Nonce()),
		To:       Addr2QctStrPtr(tx.To()),
		SubId:    UUIDLetStrPtr(tx.SubId()),
...
func (s *PublicTransactionPoolAPI) GetTransactionCount(ctx context.Context, address string, blockNr rpc.BlockNumber) (*hexutil.Uint64, error) {
	state, _, err := s.b.StateAndHeaderByNumber(ctx, blockNr)
	if state == nil || err != nil {
		return nil, err
	}
	nonce := state.GetNonce(str2Addr(address))
...
// //gasPrice is fixed, gasLimit = fee / gasPrice
// func (args *LetSendTxArgs) fee2GasLimit(fee *hexutil.Big) uint64 {
...
func (args *SendTxArgs) setDefaults(ctx context.Context, b Backend) error {
	if args.Fee == nil {
		return errors.New(`missing fee field`)
	}

	oneqct := new(big.Int).Mul(big.NewInt(1), big.NewInt(params.QCT)) // 1 QCT
	minqct := new(big.Int).Div(oneqct, big.NewInt(100))                 // 1/100 = 0.01QCT
	maxqct := new(big.Int).Mul(oneqct, big.NewInt(1))                   // 1QCT

	feeValue := (*hexutil.Big)(args.Fee)
	if feeValue.ToInt().Cmp(minqct) < 0 {
		return errors.New(`fee < 0.01 QCT`)
	}

	if feeValue.ToInt().Cmp(maxqct) > 0 {
		return errors.New(`fee > 1 QCT`)
	}
	...
	//gasPrice is fixed, gasLimit = fee / gasPrice, fee ∈ [0.01,1] QCT
	args.GasPrice = (*hexutil.Big)(big.NewInt(100000000000))

	limit := new(big.Int).Div(feeValue.ToInt(), args.GasPrice.ToInt())
	*(*uint64)(args.Gas) = limit.Uint64()
	//}
	if args.Value == nil {
		args.Value = new(hexutil.Big)
	}
	if args.Nonce == nil {
		nonce, err := b.GetPoolNonce(ctx, str2Addr(args.From))
		if err != nil {
			return err
		}
	...
	if args.SubId == nil {
		args.SubId = &common.ZeroUUIDString
	}
...
func (s *PublicTransactionPoolAPI) LoginCandidate(ctx context.Context, from common.Address) (common.Hash, error) {
	var args SendTxArgs
	args.From = from.Hex()
	args.To = Addr2QctStrPtr(&from)

	oneqct := new(big.Int).Mul(big.NewInt(1), big.NewInt(params.QCT)) // 1 QCT
	minqct := new(big.Int).Div(oneqct, big.NewInt(100))                 // 1/100 = 0.01QCT

	args.Fee = new(hexutil.Big)
	args.Fee = (*hexutil.Big)(minqct)
...
//
// https://github.com/quickchain/wiki/wiki/JSON-RPC#qct_sign
func (s *PublicTransactionPoolAPI) Sign(addr string, data hexutil.Bytes) (hexutil.Bytes, error) {
	// Look up the wallet containing the requested signer
	account := accounts.Account{Address: str2Addr(addr)}
...
func (s *PublicTransactionPoolAPI) SignTransaction(ctx context.Context, args SendTxArgs) (*SignTransactionResult, error) {
	...
	tx, err := s.sign(str2Addr(args.From), args.toTransaction())
	if err != nil {
		return nil, err
	}
...
func (s *PublicTransactionPoolAPI) Resend(ctx context.Context, sendArgs SendTxArgs, gasPrice *hexutil.Big, gasLimit *hexutil.Uint64) (common.Hash, error) {
	...
	for _, p := range pending {
		...
		if pFrom, err := types.Sender(signer, p); err == nil && pFrom == str2Addr(sendArgs.From) && signer.Hash(p) == wantSigHash {
			...
			signedTx, err := s.sign(str2Addr(sendArgs.From), sendArgs.toTransaction())
			if err != nil {
				return common.Hash{}, err
			}
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", dpos.SeedHash(number)), nil
}
...
// Version returns the current quickchain protocol version.
