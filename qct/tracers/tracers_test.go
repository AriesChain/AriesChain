...
var makeTest = function(tx, rewind) {
  ...
  var block   = let.getBlock(let.getTransaction(tx).blockHash);
  var genesis = let.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.let.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  let.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
