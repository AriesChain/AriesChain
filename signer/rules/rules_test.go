...
func (alwaysDenyUi) OnApprovedTx(tx qctapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUi) OnApprovedTx(tx qctapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(qctapi.SignTransactionResult{})
...
func dummySigned(value *big.Int) *types.Transaction {
	...
	data := make([]byte, 0)
	return types.NewTransaction(types.Binary, 3, to, value, gas, gasPrice, data)
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := qctapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx qctapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
