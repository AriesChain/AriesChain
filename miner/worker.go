...
func (self *worker) commitNewWork() {
	...
	work.commitTransactionsMining(self.mux, txs, self.chain, self.coinbase, header)
...
func (env *Work) commitTransactionsMining(mux *event.TypeMux, txs *types.TransactionsByPriceAndNonce, bc *core.BlockChain, coinbase common.Address, header *types.Header) {
	gp := new(core.GasPool).AddGas(env.header.GasLimit)
	...
	for {
		...
		if err := env.worker.engine.JudgeTx(env.worker.chain, header, tx, from); err != nil {
			log.Error("ignore the create contract Transaction from a no authorized account for clique engine", "err", err)
			txs.Pop()
			env.worker.eth.TxPool().RemoveTx(tx.Hash(), true)
			continue
		}

		log.Trace("JudgeTx mining", "from", from)
